---
title: "Explore Siamese Networks for Self-Explaining Neural Networks"
header:
...
*by [Carina Schnuck](), Massimo Höhn und [Edward Günther]()*

In the [last post](../SENN) we have talked about [Self-Explaining Neural Network (*SENN*) from David Alvarez-Melis and Tommi Jaakkola](https://arxiv.org/abs/1806.07538) [[1]](#1) and extended the model by a new conceptizer based on supervised <span>$\beta$</span>-variational autoencoders (*VaeSENN*). As mentioned in the [last post](SENN), with the *VaeSENN* we aimed to overcome the following shortcomings of an autoencoder used by Alvarez-Melis and Jaakkola for the  conceptizer in the SENN:
...
While the *VaeSENN* addresses all of these issues, [[4]](#4) correctly point out that a crucial part of interpretability for a *SENN* depends also on the robustness of the conceptizer itself. That is, the conceptizer should also be relatively stable for close inputs with the same class label (similar to the local interpretability requirement demanded on the parametrizer). Thus, it should for example not be possible to slightly change the image of a four and obtain concepts corresponding to an image of a five.

In this blog post we will present another extension of a Self-Explaining Neural Network [[1]](#1) in order to enhance interpretability-robustness while keeping all other properties of the *VaeSENN*.

## Siamese Networks
Before diving into our model we have to briefly discuss Siamese networks. If you are already familiar with Siamese networks just skip this section and head straight to the next.

Siamese networks are a class of neural networks that consist of two (or more) identical subnetworks that have the same configuration and share weights. The aim of these networks is to evaluate the similarity of different inputs by comparing its feature vectors, which is why these networks are used in many applications including signature and face verification, one-shot learning, and others.

The main problem of Siamese networks is that they have a tendency towards collapsing solutions. For example: imagine we would like to train a Siamese network to determine whether two different images from MNIST represent the same digit or not. We could train a Siamese network consisting of two different subnetworks with pairs of images from the same class and minimize the Euclidean distance between the two feature vectors as a loss. The problem is that the network could set all input weights to zero. By doing so the network collapses to a constant solution which minimizes the Euclidean distance but would never detect images from different classes. Therefore, we need to subject the training of Siamese networks to certain conditions to avoid collapsing solutions.
...
The proposed network architecture (Figure 1) and training procedure is described in the following. During training our architecture takes as an input three images: (i) the image <span>$x\_1$</span> to be classified, (ii)  in-class example <span>$x\_2$</span>, a sampled image of the same class, and (iii) out-of-class  example  <span>$x\_3$</span>, a sampled image of a different class. The three images are processed by the same probabilistic encoder <span>$q\_{\phi}(z|x)$</span> (the conceptizer in the *SENN* framework).
...
3. adversarial out-class distance: for the given data point <span>$x$</span> we search for two other input points that are close to <span>$x$</span> where one input point has the same predicted class as <span>$x$</span> and the other a different class (in case you are interested in adversarial attacks: we use PGD attack TODO INSERT LINK HERE and measure closeness with an <span>$L\_\infty$</span>- ball of size <span>$\epsilon = 0.2$</span>). 
 
For the concepts to be interpretable, these should also be robust in the sense that small perturbations of concepts should not change the predicted class. This is measured in particular by the adversarial out-class distance. In-class and out-class distances are calculated to give a scale on which the adversarial out-class distance can be compared across models.
